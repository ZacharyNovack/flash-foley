...
def generate_cond(
    ...
    # --- Feature extraction from conditioning_audio ---
    input_add_cond = None
    pitch_curve_plot = None
    if conditioning_audio is not None:
        # conditioning_audio: (sr, np.ndarray)
        in_sr, audio_arr = conditioning_audio
        import torch
        from torchaudio import transforms as T
        # Convert to torch tensor
        print(audio_arr.shape, audio_arr.dtype)
        if audio_arr.dtype == np.float32:
            audio_tensor = torch.from_numpy(audio_arr)  # [n, 2] -> [2, n]
        elif audio_arr.dtype == np.int16:
            audio_tensor = torch.from_numpy(audio_arr).float().div(32767)
        elif audio_arr.dtype == np.int32:
            audio_tensor = torch.from_numpy(audio_arr).float().div(2147483647)
        else:
            raise ValueError(f"Unsupported audio data type: {audio_arr.dtype}")
        ...
        if in_sr != sample_rate:
            resample_tf = T.Resample(in_sr, sample_rate).to(audio_tensor.device).to(audio_tensor.dtype)
            audio_tensor = resample_tf(audio_tensor)

        # Pad/crop to input_sample_size
        if audio_tensor.shape[-1] < input_sample_size:
            pad = input_sample_size - audio_tensor.shape[-1]
            audio_tensor = torch.nn.functional.pad(audio_tensor, (0, pad))
        elif audio_tensor.shape[-1] > input_sample_size:
            audio_tensor = audio_tensor[..., :input_sample_size]

        # --- Feature extraction ---
        from pre_encode import pesto_pitch_256, a_weighted_gpu, spectral_centroid
        features = extract_features(audio_tensor, sample_rate=sample_rate)

        # Optionally apply non-causal median filter to all features
        if median_filter_width and median_filter_width > 0:
            ...
            features["pesto_pitch"] = noncausal_median_filter(features["pesto_pitch"].unsqueeze(0), kernel_size=median_filter_width).squeeze(0)
            if not use_pitch_cond:
                features["pesto_pitch"] = torch.zeros_like(features["pesto_pitch"])

            features["rms_energy"] = noncausal_median_filter(features["rms_energy"].unsqueeze(0), kernel_size=median_filter_width).squeeze(0)
            if not use_rms_cond:
                features["rms_energy"] = torch.ones_like(features["rms_energy"]) * (-100)

            features["spectral_centroid"] = noncausal_median_filter(features["spectral_centroid"].unsqueeze(0), kernel_size=median_filter_width).squeeze(0)
            if not use_centroid_cond:
                features["spectral_centroid"] = torch.zeros_like(features["spectral_centroid"])

            print(f"Pitch feature after median filter shape: {features['pesto_pitch'].shape}")
            print(f"RMS feature after median filter shape: {features['rms_energy'].shape}")
            print(f"Spectral centroid feature after median filter shape: {features['spectral_centroid'].shape}")

        # Model expects input_add_cond as dict
        input_add_cond = features
    ...
    og_seconds_total = seconds_total
    if chunk_size > 0:
        # if we are doing chard, we need to set the seconds_total parameter to whatever the next highest value is
        # in the chunk size
        seconds_total = min(math.ceil(chunk_size * model.pretransform.downsampling_ratio / sample_rate) + 4, 11) # this is buffer so that the audio doesn't fade out
    ...
    if input_add_cond is not None:
        for k, v in input_add_cond.items():
            conditioning_dict[k] = v
            assert type(v) == torch.Tensor, f"{k} is a {type(v)}, {v}"
    ...
    cfg_scale = [cfg_scale1, cfg_scale2] if cfg_scale2 > 0.0 else cfg_scale1
    ...
    if chunk_size > 1:
        generate_args["chunk_size"] = chunk_size
    if stride > 1:
        generate_args["stride"] = stride
    if overlap_depth > 1:
        generate_args["overlap_depth"] = overlap_depth
    if mode is not None:
    ...
    if cut_to_seconds_total:
        audio = audio[:,:,:og_seconds_total*sample_rate]


    # --- After generation: extract features from generated audio and plot ---
    # generated_multiplot = None
    if show_pitch_curve:
        import matplotlib.pyplot as plt
        import io
        import PIL.Image
        # audio: (channels, samples) after rearrange and conversion below
        # Use the first channel for feature extraction
        audio_for_features = audio[0]
        if audio_for_features.ndim == 2 and audio_for_features.shape[0] > 1:
            audio_for_features = audio_for_features.mean(dim=0, keepdim=True)
        elif audio_for_features.ndim == 1:
            audio_for_features = audio_for_features.unsqueeze(0)
        # Extract features
        print(f"Extracting features from generated audio of shape: {audio_for_features.shape}")
        gen_features = extract_features(audio_for_features.to(device), sample_rate=sample_rate)
        # Multiplot
        gen_pitch_argmax = gen_features["pesto_pitch"][0].argmax(dim=0).cpu().numpy()
        gen_rms_plot = gen_features["rms_energy"].squeeze().cpu().numpy() if gen_features["rms_energy"].ndim > 1 else gen_features["rms_energy"].cpu().numpy()
        gen_centroid_plot = gen_features["spectral_centroid"].squeeze().cpu().numpy() if gen_features["spectral_centroid"].ndim > 1 else gen_features["spectral_centroid"].cpu().numpy()
        # fig, axs = plt.subplots(1, 3, figsize=(16, 3))
    #     axs[0].plot(gen_pitch_argmax, lw=1)
    #     axs[0].set_title("Generated Pitch (bin index)")
    #     axs[0].set_xlabel("Frame")
    #     axs[0].set_ylabel("Pitch bin")
    #     axs[0].set_ylim(0, gen_features["pesto_pitch"].shape[1]-1)
    #     axs[1].plot(gen_rms_plot, lw=1, color='orange')
    #     axs[1].set_title("Generated RMS (A-weighted dB)")
    #     axs[1].set_xlabel("Frame")
    #     axs[1].set_ylabel("RMS (dB)")
    #     axs[2].plot(gen_centroid_plot, lw=1, color='green')
    #     axs[2].set_title("Generated Spectral Centroid (scaled)")
    #     axs[2].set_xlabel("Frame")
    #     axs[2].set_ylabel("Centroid (0-1)")
    #     plt.tight_layout()
    #     buf = io.BytesIO()
    #     plt.savefig(buf, format="png")
    #     plt.close(fig)
    #     buf.seek(0)
    #     generated_multiplot = PIL.Image.open(buf)
    ...
    # Return a single multiplot image if show_pitch_curve is enabled
    if show_pitch_curve:
        import matplotlib.pyplot as plt
        import io
        import PIL.Image
        # Compose both multiplots into a single 2-row, 3-column plot
        fig, axs = plt.subplots(2, 3, figsize=(16, 6))
        # Conditioning
        pitch_argmax = features["pesto_pitch"].argmax(dim=0).cpu().numpy()
        rms_plot = features["rms_energy"].squeeze().cpu().numpy() if features["rms_energy"].ndim > 1 else features["rms_energy"].cpu().numpy()
        centroid_plot = features["spectral_centroid"].squeeze().cpu().numpy() if features["spectral_centroid"].ndim > 1 else features["spectral_centroid"].cpu().numpy()
        axs[0,0].plot(pitch_argmax, lw=1)
        axs[0,0].set_title("Cond Pitch (bin index)")
        axs[0,0].set_xlabel("Frame")
        axs[0,0].set_ylabel("Pitch bin")
        axs[0,0].set_ylim(0, features["pesto_pitch"].shape[0]-1)
        axs[0,1].plot(rms_plot, lw=1, color='orange')
        axs[0,1].set_title("Cond RMS (A-weighted dB)")
        axs[0,1].set_xlabel("Frame")
        axs[0,1].set_ylabel("RMS (dB)")
        axs[0,2].plot(centroid_plot, lw=1, color='green')
        axs[0,2].set_title("Cond Spectral Centroid (scaled)")
        axs[0,2].set_xlabel("Frame")
        axs[0,2].set_ylabel("Centroid (0-1)")
        # Generated
        gen_pitch_argmax = gen_features["pesto_pitch"].argmax(dim=0).cpu().numpy()
        gen_rms_plot = gen_features["rms_energy"].squeeze().cpu().numpy() if gen_features["rms_energy"].ndim > 1 else gen_features["rms_energy"].cpu().numpy()
        gen_centroid_plot = gen_features["spectral_centroid"].squeeze().cpu().numpy() if gen_features["spectral_centroid"].ndim > 1 else gen_features["spectral_centroid"].cpu().numpy()
        axs[1,0].plot(gen_pitch_argmax, lw=1)
        axs[1,0].set_title("Gen Pitch (bin index)")
        axs[1,0].set_xlabel("Frame")
        axs[1,0].set_ylabel("Pitch bin")
        axs[1,0].set_ylim(0, gen_features["pesto_pitch"].shape[0]-1)
        axs[1,1].plot(gen_rms_plot, lw=1, color='orange')
        axs[1,1].set_title("Gen RMS (A-weighted dB)")
        axs[1,1].set_xlabel("Frame")
        axs[1,1].set_ylabel("RMS (dB)")
        axs[1,2].plot(gen_centroid_plot, lw=1, color='green')
        axs[1,2].set_title("Gen Spectral Centroid (scaled)")
        axs[1,2].set_xlabel("Frame")
        axs[1,2].set_ylabel("Centroid (0-1)")
        plt.tight_layout()
        buf = io.BytesIO()
        plt.savefig(buf, format="png")
        plt.close(fig)
        buf.seek(0)
        multiplot_img = PIL.Image.open(buf)
        return (output_filename, [audio_spectrogram, *preview_images], multiplot_img)
    else:
        return (output_filename, [audio_spectrogram, *preview_images], None)
    ...
    with gr.Row(equal_height=False):
        with gr.Column():
            ...
            with gr.Row():
                ...
                # with gr.Column():
                cfg2_enabled_checkbox = gr.Checkbox(label="Enable CFG scale 2", value=True)
                cfg1_scale_slider = gr.Slider(minimum=0.0, maximum=25.0, step=0.1, value=default_cfg_scale, label="CFG scale 1")
                cfg2_scale_slider = gr.Slider(minimum=0.0, maximum=25.0, step=0.1, value=default_cfg_scale, label="CFG scale 2", visible=True)

                def toggle_cfg2(enabled):
                    if enabled:
                        return gr.update(visible=True)
                    else:
                        return gr.update(visible=False, value=0)

                cfg2_enabled_checkbox.change(toggle_cfg2, inputs=[cfg2_enabled_checkbox], outputs=[cfg2_scale_slider])

                # cfg1_scale_slider = gr.Slider(minimum=0.0, maximum=25.0, step=0.1, value=default_cfg_scale, label="CFG scale 1")
                # cfg2_scale_slider = gr.Slider(minimum=0.0, maximum=25.0, step=0.1, value=default_cfg_scale, label="CFG scale 2")
            ...
            with gr.Accordion("Sampler params", open=False):
                ...
                with gr.Row():
                    ...
                    if is_rf:
                        ...
                        sampler_types = ["pingpong", "pingpong-chard", "pingpong-chard-paint"]
                ...
                with gr.Row():
                    stride_slider = gr.Slider(minimum=0, maximum=256, step=1, value=0, label="Stride (0 is off)")
                    chunk_size_slider = gr.Slider(minimum=0, maximum=256, step=1, value=0, label="Chunk Size (0 is off)")
                    overlap_depth_slider = gr.Slider(minimum=0, maximum=25, step=1, value=0, label="Overlap Depth (0 is off)")
                    mode_dropdown = gr.Dropdown([None, "naive", "multidiffusion"], label="Mode", value=None)
            ...
            with gr.Accordion("Conditioning audio (mic/file)", open=False):
                conditioning_audio_input = gr.Audio(label="Conditioning audio (mic/file)", waveform_options=gr.WaveformOptions(show_recording_waveform=True))
                median_filter_width_slider = gr.Slider(minimum=1, maximum=13, step=1, value=1, label="Median filter width (1=off)")
                show_pitch_curve_checkbox = gr.Checkbox(label="Show Controls", value=False)
                with gr.Row():
                    pitch_checkbox = gr.Checkbox(label="Use pitch conditioning", value=True, visible=True)
                    rms_checkbox = gr.Checkbox(label="Use loudness conditioning", value=True, visible=True)
                    centroid_checkbox = gr.Checkbox(label="Use spectral centroid conditioning", value=True, visible=True)
            ...
            inputs = [
                ...
                conditioning_audio_input,
                median_filter_width_slider,
                show_pitch_curve_checkbox,
                init_audio_input,
        ...
        with gr.Column():
            ...
            pitch_curve_output = gr.Image(label="Controls", interactive=False)
