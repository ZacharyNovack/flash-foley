...
def fast_scandir(
    ...
    # if p is not None, randomly sample p percent of the files
    if p is not None and p < 1.0:
        files = random.sample(files, int(len(files) * p))

    for dir in list(subfolders):
        sf, f = fast_scandir(dir, ext, p)
...
def keyword_scandir(
    ...
    for dir in list(subfolders):
        sf, f = keyword_scandir(dir, ext, keywords, p)
        subfolders.extend(sf)
        files.extend(f)
    return subfolders, files
...
def get_audio_filenames(
    ...
    p=None  # percentage of files to sample, if None, use all files
):
    ...
    for path in paths:               # get a list of relevant filenames
        if keywords is not None:
            subfolders, files = keyword_scandir(path, exts, keywords, p)
        else:
            subfolders, files = fast_scandir(path, exts, p)
...
class SampleDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for config in configs:
            ...
            self.filenames.extend(get_audio_filenames(config.path, keywords, p=self.p))
        ...
        try:
            ...
            for root_path in self.root_paths:
                if root_path in audio_filename:
                    # instead of relpath, can we just the final child filename
                    # info['relpath'] = audio_filename.split("/")[-1]
...
def noncausal_median_filter(x: torch.Tensor, kernel_size: int) -> torch.Tensor:
    """
    Apply a non-causal median filter (median pooling) to a 1D tensor.
    Pads both sides to preserve non-causality.
    Args:
        x: Tensor of shape (batch, length, channels)
        kernel_size: Size of the median filter window
    Returns:
        Tensor of same shape as x, median pooled non-causally.
    """
    # Move channels to second dim for windowing: (batch, length, channels) -> (batch, channels, length)
    ...
    # Move back to (batch, length, channels)
    # truncate to original length
    if median.size(-1) > x.size(-1):
        median = median[..., :x.size(-1)]
...
class PreEncodedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            controls_filename = latent_filename.replace(f".{self.latent_extension}", "_controls.pkl")
            with open(controls_filename, "rb") as f:
                controls = pkl.load(f)
                # controls is dict, put values in info
                # make sure every element in controls is a torch tensor
                # and that it has shape (seq_length, channels)
                # sometimes data might be (1, seq_length), or (1, seq_length, channels)
                for key, value in controls.items():
                    value = torch.tensor(value, dtype=torch.float32)
                    if self.latent_crop_length is not None:
                        value = value[:, start:start+self.latent_crop_length]
                    assert value.size(1) == latents.size(1), f"Control {key} has wrong length: {value.size(1)} != {latents.size(1)}"
                    # if value is 2D, add a channel dimension
                    # if len(value.shape) == 2:
                    #     value = value.unsqueeze(-1)
                    # set nans to 0
                    if key == "rms_energy":
                        # try converting decibels to more manageable values
                        # value = value / 100.0
                        pass
                ...
                if self.smooth_controls:
                    # clean_controls = {}
                    # sample a random median filter width between 1 and 13
                    kernel_size = random.randint(1, 13)
                    for key, value in controls.items():
                        if key == "pesto_pitch":
                            value[value < 0.1] = 0
                        
                        # clean_controls[key] = value
                        controls[key] = noncausal_median_filter(value.unsqueeze(0), kernel_size).squeeze(0)

                        assert controls[key].size(1) == latents.size(1), f"Control {key} has wrong length after smoothing: {controls[key].size(1)} != {latents.size(1)}"
                        # if key is "pesto_pitch", zero out probs below .1

                        # try to load in controls, which will be filename_controls.
                    # controls['clean_controls'] = clean_controls


            info.update(controls)
...
def create_dataloader_from_config(dataset_config, batch_size, sample_size, sample_rate, audio_channels=2, num_workers=4, shuffle = True):
    ...
    if dataset_type == "audio_dir":
        ...
        smooth_controls = dataset_config.get("smooth_controls", False)
