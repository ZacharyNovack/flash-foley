...
class ExtractedTensorConditioner(Conditioner):
    """Conditioner that takes an input and returns it as the conditioning output.
    This is useful for controls that have already been extracted and need to be passed through the model without modification.
    """
    def __init__(self, 
                 output_dim: int, 
                 project_out: bool = False,
    ...
    def forward(self, tensors: tp.List[torch.Tensor], device: tp.Any = "cuda") -> tp.Any:
        self.proj_out.to(device)

        # Stack tensors along the batch dimension
        stacked_tensors = torch.stack(tensors, dim=0).to(device)
        # If min_val and max_val are specified, normalize the tensors
        if self.min_val is not None and self.max_val is not None:
            stacked_tensors = (stacked_tensors - self.min_val) / (self.max_val - self.min_val)
            stacked_tensors = stacked_tensors.clamp(0, 1)

        # Cast to same type as proj_out, unless proj_out is Identity
        if not isinstance(self.proj_out, nn.Identity):
            proj_out_dtype = next(self.proj_out.parameters()).dtype
            stacked_tensors = stacked_tensors.to(proj_out_dtype)

        return self.proj_out(stacked_tensors)
...
def create_multi_conditioner_from_conditioning_config(config: tp.Dict[str, tp.Any], pretransform=None) -> MultiConditioner:
    ...
    for conditioner_info in config["configs"]:
        ...
        elif conditioner_type == "extracted":
            conditioners[id] = ExtractedTensorConditioner(**conditioner_config)
