...
def block_causal_mask(b, h, q_idx, kv_idx, block_size):
    return (q_idx // block_size) >= (kv_idx // block_size)

# positional embeddings

class KVCache(nn.Module):
    def __init__(self, max_batch_size, max_seq_length, n_head, head_dim, dtype, device, is_init=True):
        super().__init__()
    ...
    def update(self, k_val, v_val, input_pos=None):
        # input_pos: [S], k_val: [B, H, S, D]
        if self.k_cache.dtype != k_val.dtype:
            self.k_cache = self.k_cache.to(k_val.dtype)
        if self.v_cache.dtype != v_val.dtype:
            self.v_cache = self.v_cache.to(v_val.dtype)
        k_out = self.k_cache
        v_out = self.v_cache
        if input_pos is not None:
            ...
            # we're sending the full kv cache here
            self.k_cache = k_val
            self.v_cache = v_val
            k_out, v_out = self.k_cache, self.v_cache
        self.is_init = True
        
        # self.k_cache.index_copy_(2, input_pos, k_val)   # scatter-copy ──> no temp buffer
        # self.v_cache.index_copy_(2, input_pos, v_val)
        # return self.k_cache, self.v_cache
        max_pos = input_pos.max() + 1 if input_pos is not None else k_val.shape[2]
        k_out_pos = k_out[:, :, :max_pos]
        v_out_pos = v_out[:, :, :max_pos]

        return k_out_pos, v_out_pos
...
class Attention(nn.Module):
    ...
    def convert_qkv_to_q_and_kv(self):
        '''
        Converts the qkv linear layer to separate q and kv linear layers.
        We need this for effective kv caching.
        '''
        if hasattr(self, 'to_qkv'):
            self.to_q = nn.Linear(self.dim, self.dim, bias=False)
            self.to_kv = nn.Linear(self.dim, self.dim * 2, bias=False)
            # update the weights
            self.to_q.weight.data = self.to_qkv.weight.data[:, :self.dim]
            self.to_kv.weight.data = self.to_qkv.weight.data[:, self.dim:]
            # delete the qkv layer
            del self.to_qkv
    ...
    @compile
    def apply_q_layernorm(self, q):
        q_type = q.dtype
        q = self.q_norm(q).to(q_type)
        return q


    def apply_attn(self, q, k, v, causal = None, flex_attention_block_mask = None, flex_attention_score_mod = None, flash_attn_sliding_window = None, input_pos = None):
        ...
        if causal and (flex_attention_block_mask is not None or flex_attention_score_mod is not None):
            # print(f"Disabling 'Disabling FlexAttention because causal is set', here be dragons")
            # flex_attention_block_mask = None
            # flex_attention_score_mod = None
            pass
        
        if USE_CUDNN_SDPA_BACKEND:
            with sdpa_kernel(SDPBackend.CUDNN_ATTENTION): 
                out = F.scaled_dot_product_attention(
                    q, k, v,
                    attn_mask=flex_attention_block_mask,
                    is_causal=False
                )
            ...
            # if input_pos is not None:
            #     input_pos_max_bs = input_pos.max() * torch.ones(q.shape[0], device=q.device, dtype=torch.int32)
            ...
            #     out = flash_attn_with_kvcache(q, k, v, cache_seqlens=input_pos_max_bs)
            # else:
            out = flash_attn_func(q, k, v, causal = causal, window_size=flash_attn_sliding_window if (flash_attn_sliding_window is not None) else [-1,-1])
    ...
    def forward(
        ...
        use_cross_kv_cache = has_context and self.kv_cache is not None and self.kv_cache.is_init
        ...
        if self.qk_norm == "l2":
            ...
            if use_cross_kv_cache:
                q = self.apply_q_layernorm(q)
            else:
                q, k = self.apply_qk_layernorm(q, k)

        if rotary_pos_emb is not None:
            if input_pos is not None:
                # print(rotary_pos_emb[0].shape, input_pos)
                freqs, _ = rotary_pos_emb
                freqs = freqs[input_pos]
            else:
                freqs, _ = rotary_pos_emb
            q_dtype = q.dtype
            k_dtype = k.dtype
            ...
            # print(f'Applying rotary pos emb with freqs shape {freqs.shape} and q shape {q.shape} and k shape {k.shape}')
        ...
        if self.kv_cache is not None:
            ...
            # add new keys and values to the local cache variables
            # if update_kv_cache:
            # truncate to current position
            if input_pos is not None:
                k, v = self.kv_cache.update(k, v, input_pos)
                # k = k[:, :, input_pos, :]
                # v = v[:, :, input_pos, :]
            else:
                # print(f'Updating kv cache with full k and v tensors of shape {k.shape} and {v.shape}')
        ...
        if self.differential:
            ...
            out = self.apply_attn(q, k, v, causal = causal, flex_attention_block_mask = flex_attention_block_mask, flex_attention_score_mod = flex_attention_score_mod, flash_attn_sliding_window = flash_attn_sliding_window, input_pos = input_pos)
        ...
        if out.dtype != self.to_out.weight.dtype:
            out = self.to_out(out.to(self.to_out.weight.dtype))
        else:
            out = self.to_out(out)
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        if cross_attend:
            ...
            self.cross_attn = Attention(
                ...
                causal = False, # NOTE: cross-attention is not causal, right?
                zero_init_output=zero_init_branch_outputs,
                **attn_kwargs
            )
        ...
        if global_cond_dim is not None:
            self.to_scale_shift_gate = nn.Parameter(torch.zeros(6*dim))
            with torch.no_grad():
                self.to_scale_shift_gate[2*dim : 3*dim] = 1.0  # gate_self
                self.to_scale_shift_gate[5*dim : 6*dim] = 1.0  # gate_ff
    ...
    def forward(
        ...
        # if input_pos is not None:
        #     print(f'Using input_pos of shape {input_pos} for rotary pos emb')
        if rotary_pos_emb is None and self.add_rope:
            rotary_pos_emb = self.rope.forward_from_seq_len(x.shape[-2])

        if self.global_cond_dim is not None and self.global_cond_dim > 0 and global_cond is not None:
            to_scale_shift_gate = self.to_scale_shift_gate.unsqueeze(1)
            ...
            x = self.self_attn(x, rotary_pos_emb = rotary_pos_emb, flex_attention_block_mask = self_attention_block_mask, flex_attention_score_mod = self_attention_score_mod, flash_attn_sliding_window = self_attention_flash_sliding_window, input_pos = input_pos, causal = causal)
            x = x * gate_self
            ...
            x = x + self.self_attn_scale(self.self_attn(self.pre_norm(x), rotary_pos_emb = rotary_pos_emb, flex_attention_block_mask = self_attention_block_mask, flex_attention_score_mod = self_attention_score_mod, flash_attn_sliding_window = self_attention_flash_sliding_window, input_pos = input_pos))
...
class ContinuousTransformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if global_cond_dim is not None:
            ...
            nn.init.zeros_(self.global_cond_embedder[-1].weight)
            nn.init.zeros_(self.global_cond_embedder[-1].bias)
    ...
    def setup_causal_mask(self, seq_len, device='cuda'):
        if self.causal:
            # make block causal mask with causal block diagonal
            def b_causal_mask(b, h, q_len, kv_len):
                return block_causal_mask(b, h, q_len, kv_len, block_size=self.causal)
                    
            self.block_causal_mask = create_block_mask(b_causal_mask, B=None, H=None, Q_LEN=seq_len, KV_LEN=seq_len, device=device)

            n_blocks = (seq_len + self.causal - 1) // self.causal
            sdpa_block_causal_mask = torch.tril(torch.ones((n_blocks, n_blocks), dtype=torch.bool, device=device))
            sdpa_causal_mask = sdpa_block_causal_mask.repeat_interleave(self.causal, dim=0).repeat_interleave(self.causal, dim=1)[:seq_len,:seq_len]
            self.sdpa_causal_mask = sdpa_causal_mask


    def setup_caches(self, batch_size, seq_len, cross_seq_len, dtype=torch.float32, device='cuda'):
        """setup kv_cache and causal attention mask for the transformer"""
        for layer in self.layers:
            if hasattr(layer, 'self_attn') and layer.self_attn.kv_cache is None:
                layer.self_attn.kv_cache = KVCache(
                    max_batch_size = batch_size,
                    max_seq_length = seq_len,
                    n_head = layer.self_attn.num_heads,
                    head_dim = self.dim // layer.self_attn.num_heads,
                    dtype = dtype,
                    device = device
                )
                # convert qkv to q and kv for effective kv caching
                # layer.self_attn.convert_qkv_to_q_and_kv()
            if hasattr(layer, 'cross_attn') and layer.cross_attend and layer.cross_attn.kv_cache is None:
                layer.cross_attn.kv_cache = KVCache(
                    max_batch_size = batch_size,
                    max_seq_length = cross_seq_len,
                    n_head = layer.cross_attn.num_heads,
                    head_dim = layer.dim_context // layer.cross_attn.num_heads,
                    dtype = dtype,
                    device = device
                )
                layer.cross_attn.kv_cache.is_init = False # flag because we only need to run the kv operation on cross attn once

        # causal_mask = torch.tril(torch.ones(seq_len, seq_len, dtype=torch.bool)).to(device)
        # self.causal_mask = causal_mask.unsqueeze(0).repeat(batch_size, 1, 1)
        self.freqs_cis = self.rotary_pos_emb.forward_from_seq_len(seq_len) if self.rotary_pos_emb is not None else None

    def clear_caches(self):
        """clear kv_cache and causal attention mask for the transformer"""
        for layer in self.layers:
            if hasattr(layer, 'self_attn'):
                layer.self_attn.kv_cache = None
            if hasattr(layer, 'cross_attn') and layer.cross_attend:
                layer.cross_attn.kv_cache = None
                
        # self.causal_mask = None
        self.freqs_cis = None
        torch.cuda.empty_cache()

    def clear_caches_interactive(self):
        """clear kv_cache and causal attention mask for the transformer during interactive"""
        for layer in self.layers:
            if hasattr(layer, 'self_attn'):
                layer.self_attn.kv_cache.k_cache = layer.self_attn.kv_cache.k_cache * 0
                layer.self_attn.kv_cache.v_cache = layer.self_attn.kv_cache.v_cache * 0
            if hasattr(layer, 'cross_attn') and layer.cross_attend:
                layer.cross_attn.kv_cache.k_cache = layer.cross_attn.kv_cache.k_cache * 0
                layer.cross_attn.kv_cache.v_cache = layer.cross_attn.kv_cache.v_cache * 0
                layer.cross_attn.kv_cache.is_init = False
    ...
    def forward(
        ...
        **kwargs
        ...
        # if causal and training
        if self.causal and self.training:
            if (self.block_causal_mask is None or self.block_causal_mask.shape[-1] != seq):
                self.setup_causal_mask(seq, device=device)
            # kwargs["self_attention_block_mask"] = self.block_causal_mask
            kwargs["self_attention_block_mask"] = self.sdpa_causal_mask.unsqueeze(0).repeat(batch, 1, 1)[:, None]
        # elif self.causal:
        #     kwargs["self_attention_block_mask"] = self.sdpa_causal_mask.unsqueeze(0).repeat(batch, 1, 1)[:, None, kwargs['input_pos']]
        #     print("inference block mask: ", kwargs["self_attention_block_mask"].shape, self.sdpa_causal_mask.shape, kwargs["self_attention_block_mask"][0].float().mean())
