...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
            ...
            clip_grad_norm = 0.0,
            graft_teacher: Optional[AudioAutoencoder] = None,
            graft_type: Literal["strict", "loose"] = "strict",
    ):
        super().__init__()
        ...
        print("hello????????")
        if graft_teacher is not None:
            graft_teacher.to(torch.bfloat16)
            graft_fail = []
            # attempt to graft the teacher model into the autoencoder
            # so the teacher might not have the *exact* same architecture as the autoencoder
            # but the goal is to do the best shot with copying over the convolutional kernels
            if hasattr(graft_teacher, "encoder"):
                if self.autoencoder.encoder is not None and hasattr(self.autoencoder.encoder, "layers"):
                    for n, p in self.autoencoder.encoder.layers.named_parameters():
                        if n in graft_teacher.encoder.layers.state_dict():
                            print(f"Grafting encoder layer {n} from teacher model")
                            # print shapes
            
                            if p.data.shape != graft_teacher.encoder.layers.state_dict()[n].data.shape:
                                print(f"Shape mismatch for {n}: {p.data.shape} vs {graft_teacher.encoder.layers.state_dict()[n].data.shape}")
                            try:
                                if graft_type == "loose" and p.data.shape != graft_teacher.encoder.layers.state_dict()[n].data.shape:
                                    print(f"Loose grafting for {n}: {p.data.shape} vs {graft_teacher.encoder.layers.state_dict()[n].data.shape}")
                                    # try to copy as much of the data as possible
                                    # i.e. if p.data is larger than the teacher's data, copy only the first part
                                    # and if the teacher's data is larger, copy only the first part
                                    # get shape of both tensors
                                    p_shape = p.data.shape
                                    teacher_shape = graft_teacher.encoder.layers.state_dict()[n].data.shape
                                    min_shape = tuple(min(s1, s2) for s1, s2 in zip(p_shape, teacher_shape))
                                    # copy only the first part of the data, p and teacher are n-dimensional tensors
                                    # so we need to slice them accordingly
                                    p.data[tuple(slice(0, s) for s in min_shape)] = graft_teacher.encoder.layers.state_dict()[n].data[tuple(slice(0, s) for s in min_shape)]
                                else:
                                    p.data.copy_(graft_teacher.encoder.layers.state_dict()[n].data)
                            except Exception as e:
                                print(f"Failed to graft {n}: {e}")
                                graft_fail.append(f"encoder.{n}")
            if hasattr(graft_teacher, "decoder"):
                if self.autoencoder.decoder is not None and hasattr(self.autoencoder.decoder, "layers"):
                    for n, p in self.autoencoder.decoder.layers.named_parameters():
                        if n in graft_teacher.decoder.layers.state_dict():
                            print(f"Grafting decoder layer {n} from teacher model")
                            # print shapes
                            if p.data.shape != graft_teacher.decoder.layers.state_dict()[n].data.shape:
                                print(f"Shape mismatch for {n}: {p.data.shape} vs {graft_teacher.decoder.layers.state_dict()[n].data.shape}")
                            try:
                                if graft_type == "loose" and p.data.shape != graft_teacher.decoder.layers.state_dict()[n].data.shape:
                                    print(f"Loose grafting for {n}: {p.data.shape} vs {graft_teacher.decoder.layers.state_dict()[n].data.shape}")
                                    # try to copy as much of the data as possible
                                    # i.e. if p.data is larger than the teacher's data, copy only the first part
                                    # and if the teacher's data is larger, copy only the first part
                                    # get shape of both tensors
                                    p_shape = p.data.shape
                                    teacher_shape = graft_teacher.decoder.layers.state_dict()[n].data.shape
                                    min_shape = tuple(min(s1, s2) for s1, s2 in zip(p_shape, teacher_shape))
                                    # copy only the first part of the data
                                    p.data[tuple(slice(0, s) for s in min_shape)] = graft_teacher.decoder.layers.state_dict()[n].data[tuple(slice(0, s) for s in min_shape)]
                                else:
                                    p.data.copy_(graft_teacher.decoder.layers.state_dict()[n].data)
                            except Exception as e:
                                print(f"Failed to graft {n}: {e}")
                                graft_fail.append(f"decoder.{n}")
            print(f"Grafting failed for layers: {graft_fail}")

            print("Grafted teacher model into autoencoder")
            del graft_teacher
    ...
    def training_step(self, batch, batch_idx):
        ...
        # assert that the downsampling ratio is correct
        assert latents.shape[-1] == encoder_input.shape[-1] // self.autoencoder.downsampling_ratio, \
            f"Expected latent shape {encoder_input.shape[-1] // self.autoencoder.downsampling_ratio}, got {latents.shape[-1]}"
        loss_info["latents"] = latents
