...
class DiffusionCondTrainingWrapper(pl.LightningModule):
    ...
    def __init__(
        ...
        if model.model.model.global_cond_type == "adaLN":
            # initialize timestep embeddings to zero to stabilize training
            torch.nn.init.zeros_(model.model.model.to_timestep_embed[2].weight)
            torch.nn.init.zeros_(model.model.model.to_timestep_embed[2].bias)
        ...
        if self.diffusion_forcing:
            ...
            pass
            # assert self.diffusion.conditioner.mode == "input_concat", "Diffusion forcing requires input_concat mode for timestep conditioning"
...
class DiffusionCondDemoCallback(pl.Callback):
    ...
    def on_train_batch_end(self, trainer, module: DiffusionCondTrainingWrapper, outputs, batch, batch_idx):
        ...
        if self.demo_cond_from_batch:
            ...
            # for ix in range(len(demo_cond)):
            #     for k, v in demo_cond[ix]['clean_controls'].items():
            #         demo_cond[ix][k] = v
        ...
        try:
            ...
            if module.inpainting_config is not None:
                ...
                # Check for wrapped padding masks to avoid interpolation error
                first_padding_mask = demo_cond[0]["padding_mask"]
                if isinstance(first_padding_mask, list) and len(first_padding_mask) == 1:
                    padding_masks = torch.stack([md["padding_mask"][0] for md in demo_cond], dim=0).to(module.device) # Shape (batch_size, sequence_length)
                else:
                    padding_masks = torch.stack([md["padding_mask"] for md in demo_cond], dim=0).to(module.device) # Shape (batch_size, sequence_length)
                # Create a mask of random length for a random slice of the input
                ...
                if module.diffusion.pretransform is not None:
                    log_image(trainer.logger, f'demo_masked_input', tokens_spectrogram_image(inpaint_masked_input.cpu()))
                else:
                    log_image(trainer.logger, f'demo_masked_input', audio_spectrogram_image(rearrange(inpaint_masked_input, "b c t -> c (b t)").mul(32767).to(torch.int16).cpu()))
            ...
            for cfg_scale in self.demo_cfg_scales:
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if module.diffusion_objective == "v":
                        ...
                        sigmas[0] = 1.0
                        sigmas[-1] = 0.0
                        if model.model.causal:
                            # initialize kv cache
                            x = noise
                            batch_size = x.shape[0]
                            seq_len = x.shape[-1]
                            # cross_attn_seq_len = cond_inputs['cross_attn_cond'].shape[-1] if 'cross_attn_cond' in cond_inputs else 0
                            cross_attn_seq_len = 64
                            model.model.transformer.setup_caches(batch_size=batch_size, seq_len=seq_len, cross_seq_len=cross_attn_seq_len, dtype=x.dtype, device=x.device)
                            input_pos = torch.arange(model.model.causal, device=x.device, dtype=torch.int64) 
                            og_input_add_cond = deepcopy(cond_inputs.get("input_add_cond", None))

                            out = []
                            for i in range(seq_len // model.model.causal):
                                print(input_pos)
                                # truncate input_add_conds
                                for k in cond_inputs["input_add_cond"].keys():
                                    cond_inputs["input_add_cond"][k] = og_input_add_cond[k][..., input_pos]
                                # next_token = sample_flow_pingpong(model, x[..., input_pos], sigmas=sigmas, input_pos=input_pos, cache_kv=True,  **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True, use_input_add_cond_cache=True)
                                next_token = sample_flow_pingpong(model, x[..., input_pos],  input_pos=input_pos, cache_kv=True, sigmas=sigmas,  **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True, causal=False)
                                # next_token = sample_flow_pingpong(model, x[..., input_pos],  input_pos=input_pos, sigmas=sigmas,  **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True, causal=False)
                                # next_token = sample_flow_pingpong(model, x[..., input_pos], sigmas=sigmas,  **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True, causal=False)
                                
                                # next_token = sample_flow_pingpong(model, x, sigmas=sigmas,  **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True, causal=False)

                                out.append(next_token)
                                input_pos = input_pos + model.model.causal
                                # make sure to filter input_pos if it exceeds seq_len, input_pos is a tensor of size model.model.causal
                                input_pos = input_pos[input_pos < seq_len]


                            fakes = torch.cat(out, dim=-1)
                            model.model.transformer.clear_caches()
                            model.model.input_add_cond_cache = None
                            # fakes = sample_flow_pingpong(model, noise, sigmas=sigmas, **cond_inputs, cfg_scale=cfg_scale, dist_shift=module.diffusion.dist_shift, batch_cfg=True, causal=False)
