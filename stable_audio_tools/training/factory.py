...
def create_training_wrapper_from_config(model_config, model):
    ...
    if model_type == 'autoencoder':
        ...
        if training_config.get("graft_teacher_path", None) is not None:
            # load config
            print("Loading graft teacher model from config")
            from ..models.utils import load_ckpt_state_dict, copy_state_dict
            graft_teacher_config_path = training_config.get("graft_teacher_config_path", None)
            with open(graft_teacher_config_path, "r") as f:
                graft_teacher_config = json.load(f)
            # graft_teacher = load_model(ae_config["graft_teacher_path"], graft_teacher_config, model_half=True)
            graft_teacher = create_model_from_config(graft_teacher_config)

            print(f"Loading model checkpoint from {training_config['graft_teacher_path']}")
            copy_state_dict(graft_teacher, load_ckpt_state_dict(training_config["graft_teacher_path"]))

            graft_teacher.eval().requires_grad_(False)
            # graft_teacher.to(encoder.device)

            if type(graft_teacher) != type(model):
            # model is diffusion model, just get its autoencoder
                graft_teacher = graft_teacher.pretransform.model
            assert isinstance(graft_teacher, type(model)), f"Graft teacher must be an AudioAutoencoder, got {type(graft_teacher)} instead"
        else:
            graft_teacher = None
        ...
        return AutoencoderTrainingWrapper(
            ...
            graft_teacher=graft_teacher,
            graft_type=training_config.get("graft_type", "strict"),
        )
        ...
        if "arc" in training_config:
            ...
            if teacher_model_config is not None:
                ...
                if teacher_model_ckpt is not None:
                    ...
                    # get rid of pretransform to save memory
                    if hasattr(teacher_model, "pretransform"):
                        teacher_model.pretransform = None
            ...
            if discriminator_model_config is not None:
                if type(discriminator_model_config) == str:
                    with open(discriminator_model_config, "r") as f:
                ...
                if discriminator_model_ckpt is not None:
                    ...
                    # get rid of pretransform to save memory
                    if hasattr(discriminator, "pretransform"):
                        discriminator.pretransform = None
                        torch.cuda.empty_cache()
            ...
            return ARCTrainingWrapper(
                ...
                diffusion_forcing=training_config.get("diffusion_forcing", False)
            )
        ...
        from .mar import MARTrainingWrapper
        return MARTrainingWrapper(
            model,
            lr=training_config.get("learning_rate", None),
            use_ema=training_config.get("use_ema", True),
            optimizer_configs=training_config.get("optimizer_configs", None),
            ...
            timestep_sampler=training_config.get("timestep_sampler", "uniform"),
            timestep_sampler_options=training_config.get("timestep_sampler_options", {}),
            validation_timesteps=training_config.get("validation_timesteps", [0.1, 0.3, 0.5, 0.7, 0.9]),
        )
    else:
...
def create_demo_callback_from_config(model_config, **kwargs):
    ...
    if model_type == 'autoencoder':
        ...
        from .mar import MARDemoCallback

        return MARDemoCallback(
            demo_every=demo_config.get("demo_every", 2000), 
            sample_size=model_config["sample_size"],
            sample_rate=model_config["sample_rate"],
            demo_steps=demo_config.get("demo_steps", 250),
