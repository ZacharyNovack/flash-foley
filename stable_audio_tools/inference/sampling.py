...
@torch.no_grad()
def sample_flow_pingpong(model, x, steps=None, sigma_max=1, sigmas=None, callback=None, dist_shift=None,  disable=True, cache_kv=False, chunk_size=None, stride=None, prev_denoised=None, **extra_args):
    ...
    # if "inpaint_mask" in model.model.to_input_add_embed and 'inpaint_mask' not in extra_args.get("input_add_cond", {}):
    #     extra_args["input_add_cond"]["inpaint_mask"] = torch.zeros((1, 1, x.shape[-1]), device=x.device, dtype=x.dtype)
    #     extra_args["input_add_cond"]["inpaint_masked_input"] = torch.zeros_like(x)
    ...
    for i in trange(len(t) - 1, disable=disable):
        denoised = x - t[i] * model(x, t[i] * ts, **extra_args)
        if prev_denoised is not None:
            overlap = min(chunk_size - stride, x.shape[-1])
            denoised[..., :overlap] = prev_denoised[..., -overlap:]
        if callback is not None:
    ...
    if cache_kv:
        # one more model call to update kv_cache
        print("Updating kv_cache with final model call")
        _ = model(x.clone(), 0.0 * ts, **extra_args)
    if prev_denoised is not None:
        assert torch.allclose(x[:, :, :overlap], prev_denoised[..., -overlap:]), f"Overlap not matching with diff {torch.abs(x[:, :, :overlap] - prev_denoised[..., -overlap:]).max()}"
    return x


@torch.no_grad()
def sample_flow_pingpong_chard(model, x, steps=None, sigma_max=1, sigmas=None, callback=None, dist_shift=None, disable=True, cache_kv=False, stride=64, chunk_size=128, overlap_depth=None, overhang_buffer=172, mode='naive', **extra_args):
    """Chunked autoregressive diffusion sampling (CHARD) using ping-pong method."""
    assert steps is not None or sigmas is not None, "Either steps or sigmas must be provided"
    ...
    if overlap_depth is None:
        overlap_depth = len(t)
    og_input_add_cond = deepcopy(extra_args.get("input_add_cond", None))
    # Sliding window over the sequence
    end = chunk_size
    if mode == 'multidiffusion':
        prev_chunk_traj = {}
    print(f"Generating chunks with stride={stride}, chunk_size={chunk_size}, overlap_depth={overlap_depth}, overhang_buffer={overhang_buffer}")
    for start in range(0, seq_len, stride):
        if end >= seq_len:
            break
        end = min(start + chunk_size, seq_len)

        # x_chunk = torch.randn_like(x)
        x_chunk = torch.randn_like(x[..., start:end])
        ts = x_chunk.new_ones([x_chunk.shape[0]])
        for k in extra_args["input_add_cond"].keys():
            # ctrl_end = min(start + chunk_size, x_chunk.shape[-1])
            extra_args["input_add_cond"][k] = og_input_add_cond[k][..., start:end]
            # pad with zeros on the end
            # extra_args["input_add_cond"][k] = torch.nn.functional.pad(extra_args["input_add_cond"][k], (0, x_chunk.shape[-1] - (ctrl_end - start)))

        
        # Run pingpong sampling for this chunk
        for i in trange(len(t) - 1, disable=disable):
            denoised = x_chunk - t[i] * model(x_chunk, t[i] * ts, **extra_args)
            # If not the first chunk, set the first stride values to the last stride values from previous denoised chunk
            if prev_denoised is not None and i < overlap_depth:
                overlap = min(chunk_size - stride, x_chunk.shape[-1])
                if mode == "naive":
                    denoised[..., :overlap] = prev_denoised[..., -overlap:]
                elif mode == 'multidiffusion':
                    denoised[..., :overlap] = (prev_chunk_traj[i][..., -overlap:] + denoised[..., :overlap]) / 2

            if callback is not None:
                callback({'x': x_chunk, 'i': i, 't': t[i], 'sigma': t[i], 'sigma_hat': t[i], 'denoised': denoised, 'chunk_start': start, 'chunk_end': end})
            t_next = t[i + 1]
            x_chunk = (1 - t_next) * denoised + t_next * torch.randn_like(x_chunk)
            if mode == 'multidiffusion':
                prev_chunk_traj[i] = denoised.clone()[..., :chunk_size]
        if cache_kv:
            # one more model call to update kv_cache
            print(f"Updating kv_cache with final model call for chunk {start}:{end}")
            _ = model(x_chunk.clone(), 0.0 * ts, **extra_args)
        # truncate x_chunk to chunk_size
        x_chunk = x_chunk[..., :chunk_size]
        prev_denoised = x_chunk.clone()
        chunks.append(x_chunk.clone())

    return chunks


@torch.no_grad()
def sample_flow_pingpong_chard_paint(model, x, steps=None, sigma_max=1, sigmas=None, callback=None, dist_shift=None, disable=True, cache_kv=False, stride=64, chunk_size=128, overlap_depth=None, mode=None, **extra_args):
    """Chunked autoregressive diffusion sampling (CHARD) using ping-pong method."""
    assert steps is not None or sigmas is not None, "Either steps or sigmas must be provided"
    ...
    if mode == 'multidiffusion':
        prev_chunk_traj = {}
    print(f"Generating chunks with stride={stride}, chunk_size={chunk_size}, overlap_depth={overlap_depth}")
    for start in range(0, seq_len, stride):
        if end >= seq_len:
            break
        end = min(start + chunk_size, seq_len)
        ...
        if prev_denoised is not None:
            overlap = min(chunk_size - stride, x_chunk.shape[-1])
            inpaint_mask = torch.zeros((1, 1, x_chunk.shape[-1]), device=x_chunk.device, dtype=x_chunk.dtype)
            inpaint_mask[..., :overlap] = 1.0
            inpaint_masked_input = torch.zeros_like(x_chunk)
            inpaint_masked_input[..., :overlap] = prev_denoised[..., -overlap:]
            extra_args['input_add_cond']['inpaint_mask'] = inpaint_mask
            extra_args['input_add_cond']['inpaint_masked_input'] = inpaint_masked_input
            for k in extra_args["input_add_cond"].keys():
                if k not in ['inpaint_mask', 'inpaint_masked_input']:
                    extra_args["input_add_cond"][k] = (1 - inpaint_mask.float()) * extra_args["input_add_cond"][k]
        else:
            extra_args['input_add_cond']['inpaint_mask'] = torch.zeros((1, 1, x_chunk.shape[-1]), device=x_chunk.device, dtype=x_chunk.dtype)
            extra_args['input_add_cond']['inpaint_masked_input'] = torch.zeros_like(x_chunk)
...
def sample_rf(
    ...
    if sampler_type == "euler":
    ...
    elif sampler_type == "pingpong-chard":
        return sample_flow_pingpong_chard(model_fn, x, sigmas=t, sigma_max=sigma_max, callback=callback, **extra_args)
    elif sampler_type == "pingpong-chard-paint":
        return sample_flow_pingpong_chard_paint(model_fn, x, sigmas=t, sigma_max=sigma_max, callback=callback, **extra_args)
    elif sampler_type == "pingpong-speedtest":
        # run a speedtest for pingpong sampling, with 10 warmup samples and 10 samples
        # warmup
        for ix in range(10):
            _ = sample_flow_pingpong(model_fn, x, sigmas=t, sigma_max=sigma_max, callback=callback, disable=True, **extra_args)
        # actual speedtest
        ...
        elapsed_time = t0.elapsed_time(t1) # Convert to seconds
        print(f"Pingpong sampling speedtest took {elapsed_time:.2f} milliseconds for 100 samples")
        print(f"Average time per sample: {elapsed_time / 100:.2f} ms")
        # now do a normal sample
        return sample_flow_pingpong(model_fn, x, sigmas=t, sigma_max=sigma_max, callback=callback, **extra_args)
